...
var Quiz = function(quizContainer, answers) {
	 ...
	 */
	this.unansweredQuestionText = 'No respondió completamente a la pregunta';
...
Quiz.prototype.checkAnswers = function(flagUnanswered) {
	...
	for (var i=0; i < this.questions.length; i++) {
		...
		if (answerInputs.length === 0) {
			answerInputs = question.getElementsByClassName(Quiz.Classes.QUESTION_ANSWERS)[0].getElementsByTagName('select');
		}
		var input;
		...
		} else if (userAnswer.length === 0 || (userAnswer.length !== answerInputs.length && answerInputs[0].tagName === "SELECT")) {
